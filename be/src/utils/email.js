...
/**
 * Gửi email mời Hội trưởng phụ huynh review thực đơn trong thời hạn.
 * @param {object} params
 * @param {string} params.to
 * @param {string} params.fullName
 * @param {number} params.month
 * @param {number} params.year
 * @param {string|Date} params.reviewDeadlineAt
 * @param {string} params.menuUrl
 * @returns {Promise<object|null>}
 */
const sendHeadParentMenuReviewEmail = async ({
  to,
  fullName,
  month,
  year,
  ...
  const deadlineText = reviewDeadlineAt
    ? new Date(reviewDeadlineAt).toLocaleString("vi-VN")
    : "trong 48 giờ kể từ khi nhận thông báo";

  const mailOptions = {
    from: `"DucXuan Kindergarten" <${process.env.EMAIL_USER}>`,
    to,
    subject: `Yêu cầu review thực đơn Tháng ${month}/${year}`,
    html: `
      <div style="font-family: Arial, sans-serif; color:#333; line-height:1.6; max-width:640px; margin:0 auto; border:1px solid #e5e7eb; border-radius:10px; overflow:hidden;">
        <div style="background:linear-gradient(135deg,#f59e0b 0%,#d97706 100%); color:#fff; padding:18px 22px;">
          <h2 style="margin:0; font-size:20px;">Yêu cầu review thực đơn</h2>
        </div>
        <div style="padding:22px; background:#fff;">
          <p>Xin chào <strong>${fullName || "Hội trưởng phụ huynh"}</strong>,</p>
          <p>Ban giám hiệu vừa duyệt thực đơn <strong>Tháng ${month}/${year}</strong> và cần ý kiến review từ Hội trưởng phụ huynh.</p>
          <p><strong>Thời hạn review:</strong> ${deadlineText}</p>
          <p>Nếu có ý kiến/lý do cần chỉnh sửa, vui lòng phản hồi trong thời hạn trên để hệ thống chuyển thực đơn về bếp chỉnh sửa.</p>
          <p style="margin-top:24px;">
            <a href="${menuUrl}" style="display:inline-block; padding:10px 18px; border-radius:6px; background:#f59e0b; color:#fff; text-decoration:none; font-weight:700;">
              Mở thực đơn để review
            </a>
          </p>
        </div>
        <div style="padding:14px 22px; background:#f9fafb; color:#6b7280; font-size:12px;">
          Email được gửi tự động từ hệ thống quản lý trường mầm non Đức Xuân.
        </div>
      </div>
    `,
    text: `
Xin chào ${fullName || "Hội trưởng phụ huynh"},

Ban giám hiệu vừa duyệt thực đơn Tháng ${month}/${year} và cần ý kiến review từ Hội trưởng phụ huynh.
Thời hạn review: ${deadlineText}

Mở thực đơn để review: ${menuUrl}
    `,
  };
