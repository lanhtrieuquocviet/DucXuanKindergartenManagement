...
/**
 * Build organization groups from Staff (shared by /organization-structure and /home/data).
 * Dùng cache 10 phút giống route public.
 */
const getOrganizationStructureData = async () => {
  const now = Date.now();
  ...
  staffs.forEach((item) => {
    const fullName = item?.userId?.fullName || item?.fullName;
    const userStatus = item?.userId?.status;
    const hasLinkedUser = Boolean(item?.userId);
    if (!fullName) return;
    if (hasLinkedUser && userStatus !== 'active') return;
    const member = {
...
const getOrganizationStructure = async (req, res) => {
  try {
    const now = Date.now();
    const fromCache = Boolean(orgCache.data && now < orgCache.expiry);
    const groups = await getOrganizationStructureData();
    return res.status(200).json({
      status: 'success',
      data: groups,
      fromCache,
