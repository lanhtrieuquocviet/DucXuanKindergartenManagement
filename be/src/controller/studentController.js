...
const deleteStudent = async (req, res) => {
  try {
    const { studentId } = req.params;
    const studentToDelete = await Student.findById(studentId).select('_id parentId parentProfileId');
    const parentId = studentToDelete?.parentId || null;
    const parentProfileId = studentToDelete?.parentProfileId || null;
    ...
    if (parentId) {
      const remainingChildren = await countLinkedStudentsByUserId(parentId);
      if (remainingChildren === 0) {
        const parentUser = await User.findById(parentId).select('_id roles');
        const removedAccount = await purgeOrphanParentAccount(parentUser);
        if (!removedAccount) {
          await ParentProfile.deleteOne({ userId: parentId });
          // Dọn thêm theo id để xử lý các bản ghi cũ không gắn đúng userId.
          if (parentProfileId) {
            await ParentProfile.findByIdAndDelete(parentProfileId);
          }
        }
      }
    }
...
const checkParentByPhone = async (req, res) => {
  try {
    ...
    // Dọn dữ liệu mồ côi để tránh "hồi sinh" ParentProfile sau khi đã xóa học sinh cuối.
    if (parent?._id) {
