...
function Notifications() {
  ...
  useEffect(() => {
    const loadCategory = async () => {
      try {
        ...
        const cat = list.find((c) => c.name === 'Thông báo');
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
      ...
      <h1 className="text-2xl sm:text-3xl font-bold mb-6 sm:mb-8">Thông báo</h1>
          ...
          {currentData.map((item) => (
            <div key={item.id} className="flex flex-col sm:flex-row gap-4 border-b pb-4 sm:pb-6">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs sm:text-sm text-gray-500">Thông báo | {item.date}</div>
              </div>
            </div>
          ))}
        </div>
